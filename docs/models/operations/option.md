...
```go
pagogo.WithServerURL("https://api.example.com")
```
...
```go
pagogo.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
pagogo.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
pagogo.WithTimeout(30 * time.Second)
```

...
func newWebhooks(rootSDK *Pago, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *Webhooks {
	return &Webhooks{
		rootSDK:          rootSDK,
...
func (s *Webhooks) ResetWebhookEndpointSecret(ctx context.Context, id string, opts ...operations.Option) (*operations.WebhooksResetWebhookEndpointSecretResponse, error) {
	request := operations.WebhooksResetWebhookEndpointSecretRequest{
		ID: id,
	}
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/v1/webhooks/endpoints/{id}/secret", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	res := &operations.WebhooksResetWebhookEndpointSecretResponse{
		HTTPMeta: components.HTTPMetadata{
			Request:  req,
			Response: httpRes,
		},
